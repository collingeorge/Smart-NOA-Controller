...
All simulation parameters are centralized in `config.yaml`:
...
## Educational Simulation Testing

### Synthetic Scenario Validation

This project uses entirely synthetic scenarios to test algorithm behavior. No real patient data is used or required. All profiles are simulated educational constructions.

```python
# Example: Testing with synthetic scenario parameters
patient = Patient(
    age=78, weight_kg=72, asa_class=3,
    egfr=24, allergies=[], comorbidities=["Heart Block"]
)
controller = SmartNOAController(patient)
would_have_prevented = controller.check_contraindication("Dexmedetomidine")
```

### Monte Carlo Simulation (Educational)

For educational analysis of algorithm behavior across parameter ranges:
1. **Generate synthetic parameter sets** (simulated profiles, not real patient data)
2. **Run simulations** with varied vital sign trajectories
3. **Measure intervention rates** to understand algorithm sensitivity
4. **Compare to published literature benchmarks**

**Note:** All testing uses synthetic data. No real patient records are involved.
...
- [ ] Literature rationale provided with citations
...
For potential future optimization (educational reference only):
...
## Educational Notes on Medical Device Standards

The following notes describe regulatory and integration concepts studied as part of this educational project. **This project does not implement, plan, or pursue any of these pathways.** They are included for educational reference only.

### Interoperability Standards (Educational Reference)
- HL7 FHIR is a standard for healthcare data exchange
- IEC 60601-1-8 governs medical device alarm systems
- Medical device communication requires extensive certification

### FDA Software as Medical Device (Educational Reference)

If a concept like this were ever developed into a regulated product (not the intent of this project), it would require:
- 510(k) premarket notification or PMA approval
- Design controls (ISO 13485)
- Risk management (ISO 14971)
- Prospective validation studies
- Institutional review and governance approval

**This project has none of the above. It is an educational simulation only.**

## References

Literature-based guidelines and parameters informing the simulation are derived from:
...
- **Citation**: See README.md and CITATION.cff
